...
export default function loaded() {
    ...
    function mobileNavPadding() {
        if (windowWidth() <= 800) {
            ...
            $('.header-logo--wrap').css('padding-top', `${bannerHeight}px`);
            $('.navUser').css('padding-top', `${bannerHeight}px`);
            $('.mobileMenu-toggle').css('top', `${bannerHeight}px`);
        } else {
            $('.navUser-item--cart').css('height', 'auto');
            $('.navPages-container').css('padding-top', 'inherit');
            $('.header-logo--wrap').removeAttr('style');
            $('.navUser').css('padding-top', 'inherit');
            $('.mobileMenu-toggle').css('top', '0');
        }
    }
    ...
    function toggleHeaderHeightDesktop() {
        const headerNoNavHeight = $('.header-logo--wrap').outerHeight(true) + $('.headerBanner').outerHeight(true);
    ...
    function HotelFinder() {
        // Hotel search
        $('input[name="hotelSearch"]').on('keyup', function(){
            $('.hotelResults').html("");
            ...
            if(searchString != ""){
                // show matching hotel categories
                $('.hotelNames li').each(function(){
                    hotelLowercase = $(this).text().toLowerCase();
        
                    // If match found
                    if(hotelLowercase.search(searchString) != -1){
                        if ($('.hotelResults ul').length <= 0) {
                          $('.hotelResults').append('<ul></ul>');
                        }
                        $('.hotelResults ul').append( $(this).clone() );
                    }
                });
            }
        
            if($.trim($('.hotelResults').html()) == ""){
                if(!$('.hotelResults').hasClass('noResults')) {
                    $('.hotelResults').addClass('noResults');
                }
            }else{
                $('.hotelResults').removeClass('noResults');
            }
        });
    }
    
    if ($('input[name="hotelSearch"]').length > 0) {
      HotelFinder();
    }
}
